...
func doRequest(method string, url string, body io.ReadCloser, timeout time.Duration, cookie string) (*http.Response, []byte, error) {
	req, err := newRequest(method, url, body, cookie)
	if err != nil {
		return nil, nil, err
	}
...
func newRequest(method string, url string, body io.ReadCloser, cookie string) (*http.Request, error) {
