...
var (
	BaseURL               = "https://archive.vn"                                                                                                       // Overrideable default package value.
	HTTPHost              = "archive.vn"                                                                                                               // Overrideable default package value.
...
func Capture(u string, cookie string, cfg ...Config) (string, error) {
	...
	} else if submitID, err = newSubmitID(timeout, cookie); err != nil {
		return "", err
	}
	...
	resp, body, err = doRequest("POST", BaseURL+"/submit/", ioutil.NopCloser(bytes.NewBufferString(content)), timeout, cookie)
	if err != nil {
		return "", err
	}
	...
	if len(cfg) > 0 && cfg[0].Wait {
		...
		if err := waitForCrawlToFinish(final, body, timeout, waitTimeout, pollInterval, cookie); err != nil {
			return final, err
		}
	}
...
func waitForCrawlToFinish(url string, body []byte, requestTimeout time.Duration, waitTimeout time.Duration, pollInterval time.Duration, cookie string) error {
	var (
	...
	for {
		...
		_, body, err = doRequest("", url, nil, requestTimeout, cookie)
